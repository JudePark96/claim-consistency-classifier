...
def tokenize_claims_and_save(raw_claim_path: str, target_path: str) -> None:
    ...
    with open(target_path, 'a+', encoding='utf-8') as f:
        for claim in claim_set:
            claim['mutated'] = ' '.join(word_tokenize(claim['mutated']))
            claim['original'] = ' '.join(word_tokenize(claim['original']))

            f.write(json.dumps(f'{claim}\n'))


if __name__ == '__main__':
    raw_claim_path = glob('../../rsc/raw_claims/*')
